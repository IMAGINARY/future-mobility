...
class CitizenRequestView {
  constructor(config) {
    ...
    this.tileColors = Object.fromEntries(
      Object.entries(CitizenRequestView.tileReferences).map(([key, type]) => [
        key,
        getTileType(this.config, type).color,
      ])
    );
  }
  ...
  displayRequest(goalId) {
    if (
      this.requests[goalId] === undefined &&
      this.config.citizenRequests[goalId] !== undefined
    ) {
      this.requests[goalId] = $("<div></div>")
        .addClass("request")
        .append(
          $("<div></div>")
            .addClass("request-person")
            .css({
        ...
        .append(
          $("<div></div>")
            .addClass("request-balloon")
            .append(
              $("<div></div>")
                .addClass("request-text-de")
                .html(
                  this.formatRequestText(this.config.citizenRequests[goalId].de)
                )
            )
            .append(
              $("<div></div>")
                .addClass("request-text-en")
                .html(
                  this.formatRequestText(this.config.citizenRequests[goalId].en)
                )
            )
        )
  ...
  getRandomCitizenIcon(goalId) {
    ...
    const icons = urgent
      ? this.config.citizenIcons.urgent
      : this.config.citizenIcons.regular;
  ...
  formatRequestText(text) {
    return text
      .replaceAll(
        CitizenRequestView.tileRefRegexp,
        (match, tileSpec, innerText) =>
...
<span class="tileref-stub" style="background-color: ${this.tileColors[tileSpec]}"></span> ${innerText}
      ...
      .replaceAll(
        CitizenRequestView.largeTextRegexp,
...
CitizenRequestView.tileRefRegexp = new RegExp(
  `([${Object.keys(CitizenRequestView.tileReferences).join(
    ""
  )}])\\[([^\\]]+)\\]`,
  "g"
);
